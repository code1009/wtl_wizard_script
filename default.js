...
function PopupDebugMessage(m)
{
	...
	shell.Popup(m, timeout, "error", buttons + icon);
}
...
//PopupDebugMessage(strWizTempFile);

//		var WizTempFile = fso.GetFile(strWizTempFile);
//		return WizTempFile;
		return strWizTempFile;
	}
	catch(e)
	{
		PopupDebugMessage("8");
...
function AddFilesToCustomProj(proj, strProjectName, strProjectPath, InfFile)
{
	try
	{
		...
		//var strTextStream = InfFile.OpenAsTextStream(1, 0);
		// ef bb bf utf8 bom
		//var strBOM = strTextStream.Read(2);

var stream = new ActiveXObject("ADODB.Stream");
stream.Type = 2; // 텍스트 모드
stream.Charset = "utf-8";
stream.Open();
stream.LoadFromFile(InfFile);
